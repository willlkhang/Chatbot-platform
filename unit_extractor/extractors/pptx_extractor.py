...
Extracts slide text and speaker notes and returns one `TextData`
instance per slide. Small docstrings and comments were added to clarify
...
class PptxExtractor(ExtractorBase):
    """Extract textual content and notes from `.pptx` slides."""
    ...
    def _get_text(self, slide) -> str:
        """Collect text from all shapes on the slide in visual order."""
        collected_text = []
    ...
    def _get_notes(self, slide) -> str:
        """Return speaker notes if present, otherwise an empty string."""
