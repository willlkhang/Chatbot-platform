"""Line-delimited JSON serializer for `TextData` instances.

Writes one JSON object per line; this format is convenient for streaming
and downstream processing. The function accepts a list of `TextData`
objects and a `save_path` to write to.
"""
...
def json_serializer(data: list[TextData], save_path: str | Path = Path().cwd() / 'content.json'):
    ...
    # open the output file and write each TextData as a single JSON line
